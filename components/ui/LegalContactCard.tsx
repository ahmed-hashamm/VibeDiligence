...
export function LegalContactCard({ title, description, email }: LegalContactCardProps) {
  return (
    <div className="mt-20 md:mt-40 p-6 md:p-12 rounded-[2.5rem] bg-pink-500/[0.03] border border-pink-500/10 backdrop-blur-md relative overflow-hidden group">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8 md:gap-12">
        <div className="space-y-4">
          <h3 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-3 md:gap-4">
        ...
        <a href={`mailto:${email}`} className="flex-shrink-0">
          <Button className="bg-pink-500 hover:bg-pink-600 text-white font-bold h-14 md:h-16 px-6 md:px-10 rounded-2xl shadow-xl shadow-pink-500/20 transition-all hover:scale-105 active:scale-95 flex items-center gap-3 md:gap-4 group/btn">
            <span className="w-2.5 h-2.5 rounded-full bg-white animate-pulse mt-[3px] " />
            <span className="text-base md:text-lg truncate max-w-[200px] md:max-w-none">{email}</span>
          </Button>
        </a>
      </div>
    </div>
